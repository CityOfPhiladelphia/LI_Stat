        ...
        FROM
            (
                SELECT DISTINCT
                    ...
                    EXTRACT(MONTH FROM tlic.lastissuedate) issuemonth,
                    EXTRACT(YEAR FROM tlic.lastissuedate) issueyear
                FROM
                    query.o_tl_license tlic,
                    query.j_tl_application tap
                WHERE
                    tlic.objectid = tap.tradelicenseobjectid
                    AND tap.completeddate >= '01-MAR-17'
                    AND tlic.lastissuedate >= '01-MAR-17'
                    AND tap.completeddate <= SYSDATE
